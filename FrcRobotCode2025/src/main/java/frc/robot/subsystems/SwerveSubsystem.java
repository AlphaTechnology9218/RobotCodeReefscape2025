...
public class SwerveSubsystem extends SubsystemBase
{
  ...
  public SwerveSubsystem(File directory)
  {
    ...
    double angleConversionFactor = SwerveMath.calculateDegreesPerSteeringRotation(Constants.SwerveConstants.kSwerveSteeringRatio);
...
//      vision.updatePoseEstimation(swerveDrive);
  ...
  public void setupPathPlanner()
  {
      try
    {
      SwerveConstants.robotConfig = RobotConfig.fromGUISettings();
      ...
      AutoBuilder.configure(
          ...
          new PPHolonomicDriveController(
              ...
              SwerveConstants.TRANSLATION_PID,
              // Translation PID constants
              SwerveConstants.ANGLE_PID
...
// Since AutoBuilder is configured, we can use it to build pathfinding commands
  ...
  public double getMaxAngSpeed(){
    double angSpeed = swerveDrive.getMaximumChassisAngularVelocity();
    return angSpeed;
  }
  ...
  public Command driveAimAtTarget(DoubleSupplier translationX, DoubleSupplier translationY){
    return run(() -> {
      if(isValidVisionTarget()){
        System.out.println("TRYING TO ROTATE WITH: " + -getVisionRotationAngle()/70 * swerveDrive.getMaximumChassisAngularVelocity());
        swerveDrive.drive(new Translation2d(Math.pow(translationX.getAsDouble(), 3) * swerveDrive.getMaximumChassisVelocity(),
                                          ...
                                          -getVisionRotationAngle()/70 * swerveDrive.getMaximumChassisAngularVelocity(),true,false);
      }
    });
  }
